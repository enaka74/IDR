function IDD_score_list_and_param = NFL_GRN_RecordScore_RandomSampling_rescaled_yFloor(N,u)
...
for j = 1:numel(D_list)
    ...
    [tSol,YSol] = ode15s(@(t,Y) model_NFL_GRN(t,Y,param_), tRange, Y0, opts);
...
function dYdt = model_NFL_GRN(t,Y,param)
    b = param(1:2);
    r = param(3:4);
    K = param(5:7);
    h = param(8:10);
    ...
    dx = b(1)*(y/K(1))^h(1)/(1+(y/K(1))^h(1)) - x*r(1); 
