function IDD_score_list_and_param = IFFL_GRN_RecordScore_RandomSampling_rescaled_yFloor(N,u)
...
for j = 1:numel(D_list)
    ...
    [tSol,YSol] = ode15s(@(t,Y) model_IFFL_GRN(t,Y,param_), tRange, Y0, opts);
...
function dYdt = model_IFFL_GRN(t,Y,param)
    b = param(1:2);
    r = param(3:4);
    K = param(5:7);
    h = param(8:10);
    u = param(11); D = param(12);
    
    u = IFFL_input(t,u,D);
    x = Y(1); y = Y(2);
    
    dx = b(1)*(u/K(1))^h(1)/(1+(u/K(1))^h(1)) - x*r(1); 
    dy = b(2)*(u/K(2))^h(2)/(1+(u/K(2))^h(2)+(x/K(3))^h(3)) - y*r(2);
    dYdt = [dx;dy];
end
