function IDD_score_list_and_param = IFFL_NFB_signaling_RecordScore_RandomSampling_rescaled_stable(N,u)
...
a = 10 .^ (3*rand(N,3)-1); % 10^{-1 to 2}  random number from 10^0 to 10^2
b = 10 .^ (3*rand(N,1)-1); % random number from 10^0 to 10^2
K = 10 .^ (4*rand(N,4)-2); % random number from 10^-1 to 10^1
Kp = 10 .^ (4*rand(N,2)-2); % random number from 10^-1 to 10^0
...
function dYdt = model_IFFL(t,Y,param)
    a = param(1:3); b = param(4); 
    K = param(5:8); Kp = param(9:10);
    r = param(11:12); P = param(13:14);
    u = param(15); D = param(16);
    
    u = IFFL_input(t, u, D);
    x = Y(1); y = Y(2);
    
    dx = a(1)*u*(1-x) / (K(1)+u+(1-x)) + a(2)*y*(1-x) / (K(2)+y+(1-x)) - r(1)*P(1)*x / (Kp(1)+P(1)+x);
    dy = a(3)*u*(1-y) / (K(3)+u+(1-y)) - b*x*y / (K(4)+x+y) - r(2)*P(2)*y / (Kp(2)+P(2)+y);
